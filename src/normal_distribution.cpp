...
int main()
{
    ...
    { 
        //   (1)、normal_distribution::normal_distribution: Constructs a normal_distribution object, adopting the distribution parameters specified either by mean and stddev or by object parm.
        ...
        //   (6)、normal_distribution::operator(): Returns a new random number that follows the distribution's parameters associated to the object (version 1) or those specified by parm (version 2).
	    ...
	    cout << "mean: " << distribution.mean() << endl;
	    cout << "stddev: " << distribution.stddev() << endl;  
    }
	...
	//     normal_distribution<double> d2(d1.param());
    ...
    // { 
    //     // normal_distribution::reset: Resets the distribution, so that subsequent uses of the object do not depend on values already produced by it.
	//     default_random_engine generator;
	//     normal_distribution<double> distribution(0.0, 1.0);
